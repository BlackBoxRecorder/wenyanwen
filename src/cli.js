...
import { readFileSync, writeFileSync, mkdirSync, copyFileSync, watchFile } from 'node:fs';
...
function buildFiles(files, options) {
  for (const file of files) {
    try {
      ...
      // 确定输出路径
      const outputDir = options.output ? resolve(options.output) : dirname(filePath);
